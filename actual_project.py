...
# Customer Section
tk.Label(form_tab, text="Customer Name").grid(row=1, column=0, padx=10, pady=5, sticky='w')
entry_customer_name = tk.Entry(form_tab)
entry_customer_name.grid(row=1, column=1, padx=10, pady=5)

tk.Label(form_tab, text="Phone Number").grid(row=2, column=0, padx=10, pady=5, sticky='w')
entry_phone_number = tk.Entry(form_tab)
entry_phone_number.grid(row=2, column=1)

tk.Label(form_tab, text="Address").grid(row=3, column=0, padx=10, pady=5, sticky='w')
entry_address = tk.Entry(form_tab)
entry_address.grid(row=3, column=1)

tk.Button(form_tab, text="Add Customer", command=insert_customer).grid(row=4, column=0, columnspan=2, padx=10, pady=5)

# Medicine Section
tk.Label(form_tab, text="Medicine Name").grid(row=7, column=0, padx=10, pady=5, sticky='w')
entry_medicine_name = tk.Entry(form_tab)
entry_medicine_name.grid(row=7, column=1)

tk.Label(form_tab, text="Manufacturer").grid(row=8, column=0, padx=10, pady=5, sticky='w')
entry_manufacturer = tk.Entry(form_tab)
entry_manufacturer.grid(row=8, column=1)

tk.Label(form_tab, text="Category ID").grid(row=9, column=0, padx=10, pady=5, sticky='w')
entry_category_id = tk.Entry(form_tab)
entry_category_id.grid(row=9, column=1)

tk.Label(form_tab, text="Expiry Date").grid(row=10, column=0, padx=10, pady=5, sticky='w')
entry_expiry_date = tk.Entry(form_tab)
entry_expiry_date.grid(row=10, column=1)

tk.Label(form_tab, text="Quantity").grid(row=11, column=0, padx=10, pady=5, sticky='w')
entry_quantity = tk.Entry(form_tab)
entry_quantity.grid(row=11, column=1)

tk.Label(form_tab, text="Price").grid(row=12, column=0, padx=10, pady=5, sticky='w')
entry_price = tk.Entry(form_tab)
entry_price.grid(row=12, column=1)

tk.Button(form_tab, text="Add Medicine", command=insert_medicine).grid(row=13, column=0, columnspan=2, padx=10, pady=5)

# Update Section
tk.Label(form_tab, text="Customer Name").grid(row=1, column=2, padx=10, pady=5, sticky='w')
entry_customer_name = tk.Entry(form_tab)
entry_customer_name.grid(row=1, column=3)

tk.Label(form_tab, text="New Phone Number").grid(row=2, column=2, padx=10, pady=5, sticky='w')
entry_new_phone_number = tk.Entry(form_tab)
entry_new_phone_number.grid(row=2, column=3)

tk.Label(form_tab, text="New address").grid(row=3, column=2, padx=10, pady=5, sticky='w')
entry_new_address = tk.Entry(form_tab)
entry_new_address.grid(row=3, column=3)

tk.Button(form_tab, text="Update Customer", command=update_customer).grid(row=4, column=2, columnspan=2, padx=10, pady=5)

tk.Label(form_tab, text="Medicine Name").grid(row=7, column=2, padx=10, pady=5, sticky='w')
entry_customer_name = tk.Entry(form_tab)
entry_customer_name.grid(row=7, column=3)

tk.Label(form_tab, text="New Quantity").grid(row=8, column=2, padx=10, pady=5, sticky='w')
entry_new_qty = tk.Entry(form_tab)
entry_new_qty.grid(row=8, column=3)

tk.Label(form_tab, text="New Price").grid(row=9, column=2, padx=10, pady=5, sticky='w')
entry_new_price = tk.Entry(form_tab)
entry_new_price.grid(row=9, column=3)

tk.Button(form_tab, text="Update Medicine", command=update_medicine_price).grid(row=13, column=2, columnspan=2, padx=10, pady=5)

# Delete Section
tk.Label(form_tab, text="Delete Customer Name").grid(row=1, column=4, padx=10, pady=5, sticky='w')
entry_delete_customer_name = tk.Entry(form_tab)
entry_delete_customer_name.grid(row=1, column=5)

tk.Button(form_tab, text="Delete Customer", command=delete_customer).grid(row=4, column=4, columnspan=2, padx=10, pady=5)

tk.Label(form_tab, text="Delete Medicine Name").grid(row=7, column=4, padx=10, pady=5, sticky='w')
entry_delete_medicine_name = tk.Entry(form_tab)
entry_delete_medicine_name.grid(row=7, column=5)

tk.Button(form_tab, text="Delete Medicine", command=delete_medicine).grid(row=13, column=4, columnspan=2, padx=10, pady=5)

#Horizontal Seperators
tk.Label(form_tab, text="Manage Medicines", font=('Helvetica', 12),background='lightgray').grid(row=6, column=0,columnspan=6, pady=20,sticky="ew")
tk.Label(form_tab, text="Manage Customers", font=('Helvetica', 12),background='lightgray').grid(row=0, column=0,columnspan=6, pady=10,sticky="ew")
...
# Retrieve Customers
tk.Button(display_tab, text="Retrieve Customers", command=retrieve_customers).grid(row=0,column=0, columnspan=2, padx=10, pady=5)
...
# Retrieve Medicines
tk.Button(display_tab, text="Retrieve Medicines", command=retrieve_medicines).grid(row=2, column=0, columnspan=2, padx=10, pady=5)

# Treeview for displaying Medicines
medicine_tree = ttk.Treeview(display_tab, columns=("ID", "Name", "Mfg", "Exp","Qty","price","Category"), show='headings')
...
# Set column width
medicine_tree.column("ID", width=60)
medicine_tree.column("Name", width=100)
medicine_tree.column("Mfg", width=100)
medicine_tree.column("Category", width=60)
medicine_tree.column("Exp", width=80)
medicine_tree.column("Qty", width=60)
medicine_tree.column("price", width=60)
...
sub_notebook.add(low_stock_tab, text="Low Stock Medicines")
...
tree = ttk.Treeview(info_tab, columns=("col1", "col2"), show="headings")
